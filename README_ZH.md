<p align="center">
  <img src="public/logo.png" alt="洛克沙盘标志" width="120" />
</p>
...
> 💬 洛克王国：世界六对六单打对战辅助桌面应用

💡 回合推演 + 智能体多轮教练 + 伤害反推引擎 + 实战数据修正，让你在对战中少猜拳、多读牌。
...
💡 所有建议基于 **对战导论**：不是简单比伤害数字，而是从 **压制与反制对位关系**、**冗余战术**、**非冗余战术** 角度分析。
...
| 特性 | 说明 |
|------|------|
| 真·对话记忆 | 智能体记住 R2 给的建议，在 R3 能接着上文推理 |
| 开局预加载 | 第一只敌方出场就注入全队上下文（系统提示词缓存） |
| 撤销同步 | 回退回合 → 智能体对话历史同步裁掉对应轮次 |
...
- ✅ **普通回合**：1.5 秒自动确认，零打扰
- ✅ **关键事件**（KO / 首领化 / 印记层数变化 / 冰冻）→ 红色，**必须手动确认**
- ✅ **按修改键**：展开修正面板 — 改敌方血量百分比、我方血量、实际伤害、双方能量
- ✅ **智能体延迟发送**：修正完成后才触发智能体调用，保证智能体收到的是你确认过的数据
...
- ✅ 重算时复用存档的动作序列，用户的操作决策保留不变
- ✅ 智能体收到修正通知消息：`[事后修正] R3 数据已修正：敌方实际HP 60% 而非 50%`
...
| 功能页 | 说明 |
|-----|------|
| **对战** | 实时对局流程 — 我方/敌方面板 + 智能体建议 + 三栏对局分析（技能/魔法/换人） |
...
| **设置** | DeepSeek 密钥、建议模式（智能体/规则）、段位、主题、字号、数据管理 |
...
- ✅ **Node.js 18+**（开发用 24）
- ✅ **Rust**（构建 Tauri 需要）
- ✅ **Windows 10/11**（主要平台，其他桌面系统理论可用但未测试）
...
💡 没有智能体密钥时，会自动降级为规则引擎建议。要启用智能体：

1. ✅ 在 [DeepSeek 开放平台](https://platform.deepseek.com) 获取密钥（[API 教程](https://api-docs.deepseek.com/zh-cn/)）
2. ✅ 应用 → 设置 → 🤖 智能体 → 粘贴密钥
3. ✅ 添加第一只敌方精灵 → 智能体自动预加载 → 每回合后自动给出建议
...
- ✅ 快速开始（3 步上手）
- ✅ 六个功能页的完整操作流程
- ✅ 进阶功能（公式导入 / 拼音搜索 / 核对条 / 时间线 / 反推引擎 / 智能体教练）
- ✅ 快捷键一览
- ✅ 常见问题

💡 在应用内也可直接查看：教程页 → 使用教程（置顶第一项）。
...
- ✅ **B 站创作者 [卓帅丶](https://space.bilibili.com/13884095)** — 全精灵配置推荐（[《洛克王国：世界》全精灵用法分析！](https://www.bilibili.com/video/BV1Y4SfBCEwz/)）
- ✅ **DeepSeek** — 低成本多轮对话 LLM API
- ✅ **洛克王国：世界 PVP 社区** — 战术理论贡献者
- ✅ **B 站创作者 [WwlWss](https://space.bilibili.com/1972682)** 的 [战术理论](https://www.bilibili.com/video/BV12BduBCEmL/) — 核心战术框架
